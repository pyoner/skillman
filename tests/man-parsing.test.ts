...
describe("Parser - man -h", () => {
  ...
  test("should parse basic options", () => {
    const debug = parsed.options.find((o) => o.long === "debug");
    expect(debug).toBeDefined();
    expect(debug?.short).toBe("d");
    expect(debug?.description).toBe("emit debugging messages");
  });

  test("should parse options from custom sections (Main modes of operation)", () => {
    const apropos = parsed.options.find((o) => o.long === "apropos");
    expect(apropos).toBeDefined();
    expect(apropos?.short).toBe("k");
    expect(apropos?.description).toBe("equivalent to apropos");
  });

  test("should parse options from 'Finding manual pages' section", () => {
    const locale = parsed.options.find((o) => o.long === "locale");
    expect(locale).toBeDefined();
    expect(locale?.description).toBe("define the locale for this particular man search");
  });

  test("should parse options from 'Controlling formatted output' section", () => {
    const pager = parsed.options.find((o) => o.long === "pager");
    expect(pager).toBeDefined();
    expect(pager?.description).toBe("use program PAGER to display output");
  });
